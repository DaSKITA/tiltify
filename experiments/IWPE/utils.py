...
def create_learning_df(results):
    plot_dfs = []
    for result_dict in results.values():
        train_f1_scores = [train_dict["macro avg"]["f1-score"] for train_dict in result_dict["train_results"]]
        test_f1_scores = [train_dict["macro avg"]["f1-score"] for train_dict in result_dict["test_results"]]
